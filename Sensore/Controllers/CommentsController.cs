...
namespace Sensore.Controllers
{
        ...
        public async Task<IActionResult> Thread(string patientId, DateTime timestamp)
 {
   ...
   return View(comment);
        ...
        // GET: Comments/Create
        public IActionResult Create(string patientId, DateTime? threadTimestamp, long? parentCommentId)
  {
  ...
  return NotFound();
   ...
   if (comment == null)
     ...
     // Only allow the author to edit their own comment
            var currentUser = await _userManager.GetUserAsync(User);
   if (comment.AuthorUserId != currentUser.Id && !User.IsInRole("Admin"))
 {
      return Forbid();
}
...
// POST: Comments/Edit/5
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(long id, Comment comment)
  {
